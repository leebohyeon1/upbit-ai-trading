...
### 기능 활성화 위치
\`\`\`yaml
경로: 설정(Settings) → API 키 설정
위치: 왼쪽 메뉴의 설정 아이콘 클릭
\`\`\`

### API 키 설정 단계

#### 1. Alpha Vantage (주식 데이터)
\`\`\`yaml
필요성: 선택사항
용도: S&P 500, NASDAQ과의 실시간 상관관계 분석
설정 방법:
  1. https://www.alphavantage.co/support/#api-key 접속
  2. 무료 API 키 발급 (이메일만 필요)
  3. 설정 → API 키 설정 → "Alpha Vantage API Key" 입력
  4. "키 검증" 버튼 클릭하여 저장
...
#### 2. Exchange Rate API (외환 데이터)
\`\`\`yaml
필요성: 선택사항
용도: 달러 인덱스(DXY) 실시간 계산
설정 방법:
  1. https://app.exchangerate-api.com/sign-up 접속
  2. 무료 API 키 발급 (이메일만 필요)
  3. 설정 → API 키 설정 → "Exchange Rate API Key" 입력
  4. "키 검증" 버튼 클릭하여 저장
